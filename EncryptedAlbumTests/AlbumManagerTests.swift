...
final class AlbumManagerTests: XCTestCase {
    ...
    func testSuspendResumeIdleTimerUpdatesSystemIdleStateOnIOS() async throws {
            ...
            XCTAssertTrue(
                UIApplication.shared.isIdleTimerDisabled, "System idle timer should be disabled while suspended")
            ...
            XCTAssertFalse(
                ...
                "System idle timer should be re-enabled after resume when user preference is not set")
        #else
    ...
    func testPerformQuickEncryptedCloudVerificationCompletes() async throws {
            ...
            XCTAssertTrue(
                [AlbumManager.CloudVerificationStatus.success, .failed, .notAvailable].contains(
                    sut.cloudVerificationStatus))
    ...
    func testDecoyUnlockDoesNotWipeRealAlbum() async throws {
        ...
        XCTAssertTrue(
            FileManager.default.fileExists(atPath: realFileURL.path), "Real encrypted file should exist on disk")
        ...
        XCTAssertTrue(
            FileManager.default.fileExists(atPath: realFileURL.path),
            "Real encrypted file must remain on disk when in decoy mode")
    ...
    func testUserInitiatedLockSetsSuppressFlag() async throws {
        ...
        XCTAssertTrue(
            sut.suppressAutoBiometricAfterManualLock, "User-initiated lock should set suppression flag immediately")
    }
