...
class PasswordChangeJournalService {
    ...
    private let logger = Logger(subsystem: "biz.front-end.SecretVault", category: "journal")

    /// Returns the URL for the journal file in the vault directory
    private func journalURL(in vaultDirectory: URL) -> URL {
        return vaultDirectory.appendingPathComponent(journalFilename)
    }

    /// Writes the journal to disk
    func writeJournal(_ journal: PasswordChangeJournal, to vaultDirectory: URL) throws {
        let url = journalURL(in: vaultDirectory)
    ...
    func readJournal(from vaultDirectory: URL) throws -> PasswordChangeJournal? {
        let url = journalURL(in: vaultDirectory)
    ...
    func deleteJournal(from vaultDirectory: URL) throws {
        let url = journalURL(in: vaultDirectory)
    ...
    func journalExists(in vaultDirectory: URL) -> Bool {
        let url = journalURL(in: vaultDirectory)
