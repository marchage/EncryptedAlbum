...
/// Comprehensive error types for the SecretVault application
enum VaultError: LocalizedError {
    ...
    // MARK: - Vault Integrity Errors
    case vaultCorrupted(reason: String)
    ...
    var errorDescription: String? {
        ...
        case .vaultCorrupted(let reason):
            return "Vault is corrupted: \(reason)"
        case .metadataCorrupted(let reason):
            return "Vault metadata is corrupted: \(reason)"
        case .integrityCheckFailed(let reason):
            return "Vault integrity check failed: \(reason)"
        case .vaultNotInitialized:
    ...
    var recoverySuggestion: String? {
        ...
        case .vaultCorrupted, .metadataCorrupted, .integrityCheckFailed:
            return "Please restore from backup or reinitialize the vault"
        case .vaultNotInitialized:
            return "Please set up the vault first"
