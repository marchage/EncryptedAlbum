...
struct UnlockView: View {
    ...
    private func appIconView() -> some View {
        ...
        #if os(iOS)
        // Always render the high-res marketing image (1024px) at the desired display size
        // and let SwiftUI downsample for crisp retina rendering.
        ...
        if let runtime = AppIconService.shared.runtimeMarketingImage {
            return AnyView(Image(platformImage: runtime)
                ...
                .interpolation(.high)
                .aspectRatio(1, contentMode: .fit)
                .frame(maxWidth: maxVisual, maxHeight: maxVisual)
                .shadow(color: .black.opacity(0.2), radius: 10, x: 0, y: 5))
        }
