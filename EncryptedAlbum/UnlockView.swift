...
struct UnlockView: View {
    ...
    private func appIconView() -> some View {
        ...
        let selectedIcon = AppIconService.shared.selectedIconName.isEmpty ? nil : AppIconService.shared.selectedIconName
        let runtimeImage = AppIconService.shared.runtimeMarketingImage
        ...
        // Fallback to runtime image only if generation failed
        if let runtime = AppIconService.shared.runtimeMarketingImage {
            let pixels = max(runtime.size.width * runtime.scale, runtime.size.height * runtime.scale)
            AppLog.debugPublic("UnlockView: fallback to runtime image with \(Int(pixels))px")
