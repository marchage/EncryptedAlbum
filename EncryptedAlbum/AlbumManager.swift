...
// ThumbnailCache is defined in ThumbnailCache.swift (shared helper class)
...
public class AlbumManager: ObservableObject {
    ...
    func unlock(password: String) async throws {
        ...
        if isValid {
            ...
            AppLog.debugPublic(
                "unlock: key derivation completed in \(String(format: "%.3f", Date().timeIntervalSince(deriveStart)))s")
                        ...
                        Task {
                            for cap in capturesToProcess {
                                do {
                                    ...
                                    AppLog.error(
                                        "Failed to hide queued captured media \(cap.filename): \(error.localizedDescription)"
                                    )
                                }
            ...
            AppLog.debugPublic(
                "unlock: loadPhotos finished in \(String(format: "%.3f", Date().timeIntervalSince(loadStart)))s")
            await MainActor.run {
                lastActivity = Date()
            }
            startIdleTimer()
            ...
            AppLog.debugPublic(
                "unlock: finished overall unlock in \(String(format: "%.3f", Date().timeIntervalSince(overallStart)))s")
        ...
        do {
            ...
            case .data(let data):
                do {
                    ...
                    albumQueue.async { [weak self] in
                        guard let self = self else { return }
                        self.pendingCapturedMedia.append(
                            PendingCapture(
                                url: tempURL, filename: filename, dateTaken: dateTaken, sourceAlbum: sourceAlbum,
        ...
        if let encryptedThumbnailPath = photo.encryptedThumbnailPath {
            ...
            if !FileManager.default.fileExists(atPath: url.path) {
                ...
                do {
                    ...
                    AppLog.debugPrivate(
                        "[AlbumManager] decryptThumbnail: error reading encrypted thumbnail for id=\(photo.id): \(error.localizedDescription)"
                    )
                }
            }
        }
    ...
    private func loadSettings() async {
        ...
        await MainActor.run {
            ...
            if let notificationsString = settings["enableImportNotifications"],
                let notifications = Bool(notificationsString)
            {
            ...
            if let exportProtectString = settings["exportPasswordProtect"],
                let exportProtect = Bool(exportProtectString)
            {
    ...
    func authenticateAndRetrievePassword() async throws -> String {
        ...
        defer {
            ...
            AppLog.debugPublic(
                "authenticateAndRetrievePassword: prompt ended in \(String(format: "%.3f", Date().timeIntervalSince(promptStart)))s"
            )
        }
    ...
    private func loadPhotos() async throws {
        ...
        do {
            ...
            AppLog.debugPublic(
                "loadPhotos: metadata file not present or failed to read/decode: \(error.localizedDescription)")
        ...
        if lockdownModeEnabled {
            Task { @MainActor [weak self] in
                ...
                self.hideNotification = HideNotification(
                    message: "Import blocked while Lockdown Mode is enabled.", type: .failure, photos: nil)
