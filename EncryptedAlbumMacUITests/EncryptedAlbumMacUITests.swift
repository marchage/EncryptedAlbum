...
final class EncryptedAlbumMacUITests: XCTestCase {
    ...
    func testMenuContainsExpectedActions() throws {
        ...
        let menuButton = toolbarButton(
            app: app,
            identifier: "More",
            fallbackIdentifiers: ["More", "Lock Album", "ellipsis", "ellipsis.circle"])
        ...
        let lockAlbumButton = app.buttons["Lock Album"]
        let lockAlbumMenuItem = app.menuItems["Lock Album"]
        XCTAssertTrue(lockAlbumButton.exists || lockAlbumMenuItem.exists, "Lock Album should remain accessible")
